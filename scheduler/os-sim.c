...
extern void start_simulator(unsigned int new_cpu_count)
{
    ...
    /* Start CPU threads */
    for (n=0; n<cpu_count;) {
        /* mutex lock until the child thread has started */
        (void) pthread_mutex_lock(&simulator_mutex);
    
        /* create the child thread passing the cpu id (n) as parameter to
           the worker function simulator_cpu_thread_func */
        (void) pthread_create(&cpu_thread[n], NULL, simulator_cpu_thread_func,
        (void*)&n);

        /* avoid race condition on update of n with newly created child thread */
        (void) pthread_mutex_lock(&simulator_mutex);
        n++;
        (void) pthread_mutex_unlock(&simulator_mutex);
    }
...
static void *simulator_cpu_thread_func(void *data)
{
    /* stored the cpu id (n) pointed to by data into our thread variable */
    unsigned int thread = *(int *)data;

    /* release mutex lock from start_simulator now that the value of 
       n pointed to by data has been stored in an unsigned int */
    (void) pthread_mutex_unlock(&simulator_mutex);

    /* start the simulator on the new thread */
    simulator_cpu_thread(thread);
    return NULL;
}
