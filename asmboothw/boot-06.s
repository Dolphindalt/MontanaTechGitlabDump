...
# now let's iterate through the string, placing the contents of the 
# current string byte into BX, and then use that to output - 
# then update the address to the next byte continuing 
# until we encounter the null byte

    mov $0x0e, %ah   # 0xE is the video function desired
    mov $msg,  %bx   # address of first byte of msg into bx register
print_char:
    mov (%bx), %al   # move value at CX addr into AL (lower 8-bits of AX)
